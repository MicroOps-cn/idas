...
func InstallHTTPApi(logger log.Logger, container *restful.Container, options []httptransport.ServerOption, endpoints endpoint.Set) {
	...
	v1Ws.Route(v1Ws.POST("/forgotPassword").Doc("忘记用户密码").To(NewKitHTTPServer[endpoint.ForgotUserPasswordRequest](endpoints.ForgotPassword, options)).Metadata(global.MetaNeedLogin, false))
	v1Ws.Route(v1Ws.POST("/resetPassword").Doc("重置用户密码").To(NewKitHTTPServer[endpoint.ResetUserPasswordRequest](endpoints.ResetPassword, options)))
